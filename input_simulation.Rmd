...
run_simulation <- function(unbonding_stake, current_block_number) {
  ...
  # Update the back of the unbonding queue block number using the RFC logic
  GLOBAL_CURRENT_BACK_OF_UNBONDING_QUEUE_BLOCK <<- max(
    min(GLOBAL_CURRENT_BACK_OF_UNBONDING_QUEUE_BLOCK + unbonding_time_delta, 
        GLOBAL_CURRENT_BLOCK + 403200), 
    GLOBAL_CURRENT_BLOCK + 28800, 
    GLOBAL_CURRENT_BLOCK
  )
